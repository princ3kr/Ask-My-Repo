...
def _llm_review_entry_points(files: dict, flagged_paths: list[str]) -> list[dict]:
    ...
    for path in flagged_paths:
        ...
        imports_list = [imp.get("module", "") for imp in data.get("imports", [])[:15]]
        file_summaries.append(
            f"PATH: {path}\n"
            f"FUNCTIONS: {', '.join(fn_names[:20]) or '(none)'}\n"
            f"DECORATORS: {', '.join(decorators[:15]) or '(none)'}\n"
            f"IMPORTS: {', '.join(imports_list) or '(none)'}"
        )
...
def map_repository(repo_url: str, on_progress: Optional[ProgressCallback] = None):
    ...
    logger.info(f"map_repository() called for: {repo_url}")
    ...
    if qdrant_exists and neo4j_exists:
        ...
        logger.info(f"[DUPLICATE SKIP] Both Qdrant and Neo4j have data for '{repo_id}' — skipping re-index.")
    ...
    if neo4j_exists:
        ...
        logger.info(f"Graph exists for {repo_id} — skipping rebuild")
        nodes_count, edges_count = ChunkBuilder.get_repo_graph_counts(repo_id)
    else:
        report("graph_building", 22, f"Reading {file_count} files to understand the layout…")
        logger.info(f"Building graph for {repo_id}...")
        ...
        logger.info(f"Graph pushed to Neo4j for {repo_id}")
        ...
        if flagged_paths:
            ...
            logger.info(f"Flagged {len(flagged_paths)} paths for LLM entry point review")
            llm_entries = _llm_review_entry_points(files, flagged_paths)
            if llm_entries:
                ChunkBuilder.apply_entry_points(repo_id, llm_entries)
        else:
            logger.debug("No flagged entry points — skipping LLM review")
    ...
    if qdrant_exists:
        ...
        logger.info(f"Vector store exists for {qdrant_collection_name} — skipping")
    else:
        report("vector_building", 55, "Breaking code into easy-to-search pieces…")
        logger.info(f"Building vector store for {qdrant_collection_name}...")
        ...
        logger.info(f"Vector store push complete for {qdrant_collection_name}")

    report("assistant_ready", 95, "Almost ready — preparing your assistant…")
    logger.info(f"Repository indexing complete for {repo_id}!")
    return {
        "repo_id": repo_id,
        "files": files,
        "nodes_count": nodes_count,
